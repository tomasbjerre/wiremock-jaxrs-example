...
public class CreateMocksTest {
	private static Logger LOG = Logger.getLogger(CreateMocksTest.class
			.getName());

	public static String getMappingsRootPath() throws URISyntaxException {
		final Path storagePath = Paths.get(Main.class.getResource(
				"/root/mappings/.gitkeep").toURI());
	...
	public void createMocks() throws URISyntaxException {
		...
		final Options options = new WireMockConfiguration() //
				.dynamicPort() //
	...
	private void createStubs() {
		 ...
		 *
		 */
		stubFor(invocation(ExampleResource.class, (r) -> r.getItem("abc")) //
				.willReturn(aResponse().withStatus(SC_NOT_FOUND)));

		for (final ItemDTO itemDto : MockFactory.getAllItems()) {
			stubFor(invocation(ExampleResource.class,
					(r) -> r.getItem(itemDto.getAttr1())) //
					.willReturn(aResponse().withStatus(SC_ACCEPTED), itemDto));

			stubFor(invocation(ExampleResource.class,
					(r) -> r.createItem(itemDto)) //
